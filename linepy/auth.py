...
class Auth(object):
    ...
    def __loadSession(self, newgeneration=False):
        self.liff      = Session(self.server.LINE_HOST_DOMAIN, self.server.Headers, "/LIFF1").Liff()
        """ new talk and poll"""
        self.client_ntalk = TPersistentHttpClient(self.server.LINE_HOST_DOMAIN + self.server.LINE_API_QUERY_PATH_FIR)
        self.client_ntalk.setCustomHeaders(self.server.Headers)
        self.talk_transport = TBufferedTransport(self.client_ntalk)
        self.talk_protocol = TCompactProtocolAccelerated(self.talk_transport)
        self.talk_transport.open()
        self.talk = TalkService.Client(self.talk_protocol)
        #self.talk       = Session(self.server.LINE_HOST_DOMAIN, self.server.Headers, self.server.LINE_API_QUERY_PATH_FIR).Talk()
        if newgeneration:
            self.client_npoll = TPersistentHttpClient(self.server.LINE_HOST_DOMAIN + self.server.LINE_POLL_QUERY_PATH_FIR)
            self.client_npoll.setCustomHeaders(self.server.Headers)
            self.poll_transport = TBufferedTransport(self.client_npoll)
            self.poll_protocol = TCompactProtocolAccelerated(self.poll_transport)
            self.poll_transport.open()
            self.poll = TalkService.Client(self.poll_protocol)
        else:
